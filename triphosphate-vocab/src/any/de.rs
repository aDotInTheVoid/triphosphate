// https://github.com/serde-rs/json/blob/0daacdd52e4abb4a47a7f7fc598c5bd4492aa104/src/value/de.rs
...
fn visit_bytes<'de, V>(bytes: Bytes, visitor: V) -> Result<V::Value, Error>
where
    V: Visitor<'de>,
{
    todo!()
...
fn visit_link<'de, V>(link: CidLink, visitor: V) -> Result<V::Value, Error>
where
    V: Visitor<'de>,
{
    todo!()
...
impl<'de> serde::Deserializer<'de> for Value {
        ...
        V: Visitor<'de>,
    {
        match self {
            ...
            Value::List(v) => visit_array(v, visitor),
            Value::Map(v) => visit_object(v, visitor),
            Value::Bytes(v) => visit_bytes(v, visitor),
            Value::Link(v) => visit_link(v, visitor),
        }
    }
...
impl<'de> VariantAccess<'de> for VariantDeserializer {
    ...
    {
        match self.value {
            Some(Value::Map(v)) => visit_object(v, visitor),
            Some(other) => Err(serde::de::Error::invalid_type(
                other.unexpected(),
                &"struct variant",
            )),
...
impl<'de> serde::Deserializer<'de> for &'de Value {
        ...
        V: Visitor<'de>,
    {
        match self {
            ...
            Value::List(v) => visit_array_ref(v, visitor),
            Value::Map(v) => visit_object_ref(v, visitor),
            Value::Link(_) => todo!(),
            Value::Bytes(_) => todo!(),
        }
    }
...
impl<'de> VariantAccess<'de> for VariantRefDeserializer<'de> {
    ...
    {
        match self.value {
            Some(Value::Map(v)) => visit_object_ref(v, visitor),
            Some(other) => Err(serde::de::Error::invalid_type(
                other.unexpected(),
                &"struct variant",
            )),
...
impl Value {
    ...
    fn unexpected(&self) -> Unexpected {
        match self {
            ...
            Value::Bytes(b) => Unexpected::Bytes(&b.bytes),
            Value::Link(_) => Unexpected::Other("cid link"),
        }
    }
}
