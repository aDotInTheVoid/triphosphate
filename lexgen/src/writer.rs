...
pub(super) fn write_to(base: &Utf8Path, contents: &BTreeMap<ItemPath, TokenStream>) {
    ...
    for (path, file) in files.files {
        let depth = if path == "mod.rs" {
            0
        } else {
            path.components().count() as u8
        };

        let path = base.join(path);

        let contents = file.contents(depth);
...
impl File {
    fn contents(self, depth: u8) -> String {
        ...
        if depth != 0 {
            ...
            contents.push_str("use ");
            for _ in 0..depth {
                contents.push_str("super::");
            }
            contents.push_str("_lex;\n");
        } else {
        ...
        for item in self.item {
            contents.push_str(&prettyplease::unparse(&syn::parse2(item).unwrap()));
            contents.push('\n');
        }
